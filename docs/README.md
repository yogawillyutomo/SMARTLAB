...
`docs/reviews/` contains the S3.6 implementation-validation and rollout-UAT matrix at [S3.6 Offline ActivityReport Draft UAT](reviews/s3.6-offline-draft-uat.md) and the S4 closure matrix at [S4.6 Reconciliation & Storage-Cleared UAT](reviews/s4.6-reconciliation-uat.md). Both matrices separate automated implementation evidence from operator/browser checks that must be executed in the target environment.
...
| S4.6 — S4 reconciliation/UAT | **Implementation tranche / PR #83; manual browser UAT pending** | read-only Asset operational-state projection + provenance, active-custody lifecycle/unlink guard, PostgreSQL negative-stock/double-loan/Loan↔Maintenance contention gate, historical reconstruction tests, aggregate S4 source-of-truth scan, relative-doc-link CI | execute and record storage-cleared browser UAT for `/assets`, `/stock`, `/loans`, `/maintenance`; final exact-head and exact merged-head verification | S4.5 merged | S4 contract + `reviews/s4.6-reconciliation-uat.md` + source-of-truth migration | PR #83 | do not call S4 complete until manual UAT is recorded |
