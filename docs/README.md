...
| S4.5 — Preventive Maintenance | **Implementation tranche / PR #82** | exact Asset MaintenancePlan/Execution, immutable snapshots, ETag lifecycle, active Maintenance custody, symmetric Loan↔Maintenance exclusion, audited Asset condition completion, atomic Inventory issue consumption, OpenAPI 0.28, canonical `/maintenance` cutover | becomes merged authority only after exact-head review/CI + explicit merge + exact merged-head verification | S4.4 | S4 contract + current state + migration doc | PR #82 | do not merge automatically |
| S4.6 — S4 reconciliation/UAT | Planned | S4.2–S4.4 merged; S4.5 candidate in PR #82 | cross-domain custody/read-model reconciliation, contention evidence, storage-cleared browser UAT, migration/import policy if approved, final docs/merged-head verification | S4.5 merged | S4 contract + source-of-truth migration | none | begin after S4.5 merged-head verification |
