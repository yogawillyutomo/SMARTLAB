...
The `docs/reviews/` directory is not present on `main` at the verified checkpoint. Open PR #74 proposes the first review artifact at `docs/reviews/s3.6-offline-draft-uat.md`; it must not be described as merged until PR #74 actually lands.
...
| S3.6 — Offline Report Draft Sync + Operational UAT | **In progress; not merged on main** | PR #74 implements controlled offline draft working copies, idempotent server sync ledger, conflict/rebase UX, OpenAPI 0.24 and automated UAT; exact-head API/web CI PASS | operator-required authenticated-browser/DevTools network-toggle UAT; merge decision; exact merged-head verification | S3.5 | PR #74 updates current state, migration contract, Session/Report contract and proposes `docs/reviews/s3.6-offline-draft-uat.md` | open PR #74 / `0dc15565` | complete operator UAT, merge only with explicit authorization, verify merged exact head |
