...
| **SMARTLAB** | Laboratory operational aggregate; Device; Layout; Transfer; Incident; Availability; Reservation; dated Schedule Exception; Priority Event; LaboratorySession; ActivityReport; Asset; Inventory; Loan custody; future Maintenance/Telemetry | Operational mutations remain tenant-scoped, server-authorized, audited, concurrency-safe, and fail closed. |
...
| S4.4 — Loan / LoanItem custody | **Implementation tranche / PR #81** | exact Asset LoanItems, requester/view-all scope, ETag lifecycle, deterministic Asset locks, DB active-custody uniqueness, condition-out/return evidence, append-only Loan events, OpenAPI 0.27, canonical `/loans` cutover | becomes merged authority only after exact-head CI/review + explicit merge + exact merged-head verification; symmetric Maintenance exclusion starts when S4.5 creates canonical Maintenance custody | S4.3 | S4 contract + current state + migration doc | PR #81 | do not merge automatically |
| S4.5–S4.6 — remaining S4 implementation | Planned | none canonical yet | Preventive Maintenance; Loan↔Maintenance exclusion; Inventory consumption; cross-domain migration/UAT | S4.4 merged | S4 contract | none | begin S4.5 only after S4.4 merge verification |
