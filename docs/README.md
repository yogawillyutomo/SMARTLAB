...
| S5.2 — Work Order core + corrective custody | **Implementation candidate / PR #86** | WorkOrder/Event persistence, School/year numbering, exact Asset + Laboratory snapshots, optional Incident link, ETag lifecycle through completed/rework/cancel, corrective custody, Loan/PM/WorkOrder exclusion, Asset terminal guard integration, derived `in_repair`, PostgreSQL race proof, OpenAPI 0.31 | S5.3 still owns spare-part consumption + verification/Asset-condition apply; S5.4 owns frontend cutover/UAT | S5.1 locked | ADR-003 + Work Order contract + source-of-truth migration | PR #86 | exact-head CI after OpenAPI/docs reconciliation, then review; do not merge automatically |
