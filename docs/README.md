...
| S4.2 — Fixed Asset canonicalization | **Implementation tranche** | tenant-scoped Asset persistence, exact optional 1:1 Device link, lifecycle actions, ETag/audit, OpenAPI 0.25, canonical `/assets` cutover | becomes authority only when this implementation is present on merged `main`; then exact merged-head verification closes S4.2 | S4.1 | S4 contract + current state + migration doc | PR #79 | exact-head CI, review, explicit merge decision |
| S4.3–S4.6 — remaining S4 implementation | Planned | none canonical yet | immutable stock ledger; Loan custody; Preventive Maintenance; cross-domain migration/UAT | S4.2 | S4 contract | none | begin S4.3 only after S4.2 merge verification |
