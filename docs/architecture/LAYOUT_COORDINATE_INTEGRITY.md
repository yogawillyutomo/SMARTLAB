...
Stage 4C.1 did not add element-property or geometry controls. Stage 4C.2 below introduces the supported one-cell property controls, while span editing, multi-cell placement, and advanced geometry remain deferred to Stage 4C.3. Neither stage changes Asset binding or the schema/storage version.

## Stage 4C.2 custom layout element properties

Stage 4C.2 adds a selected-element inspector whose mutations are allowed only when the layout has explicitly been converted to `custom`. Named templates and Grid Classic remain informational in the inspector and continue to use the existing explicit **Ubah menjadi Custom** flow. `student_pc` and `teacher_pc` remain Device-managed and read-only, including their reference, type, label, rotation, movement flags, and spans. Explicit `empty` cells are also read-only; placement and removal remain responsibilities of the Stage 4B palette.

The centralized capability policy permits optional trimmed labels on teacher desks, printers, network switches, access points, doors, windows, and walls; label elements require non-blank text. All labels are limited to 60 characters. Visual rotation is limited to `0`, `90`, `180`, and `270` degrees for teacher desks, doors, windows, walls, aisles, and labels. Rotation transforms only the inner canvas visual; it never changes coordinates, spans, occupancy, hitboxes, or Device references.

Eligible non-PC elements expose one semantic position state rather than independent flags. Lock maps atomically to `fixed: true, movable: false`; unlock maps to `fixed: false, movable: true`. This preserves the validator invariant and leaves `swappable` untouched. An explicitly converted template door may therefore be unlocked and moved through the existing operation, while PC elements remain outside this property path.

Inspector typing is local form state. **Terapkan Properti ke Draft** invokes the pure property operation, and only a successful non-noop result updates the existing layout draft. Save and Cancel continue to use the shared baseline, dirty fingerprint, navigation protection, atomic persistence, recovery safeguards, and Device-reference validation. A successful save still emits one `layout.save` audit; its backward-compatible summary appends `propertiesChanged`, counting each existing non-empty element at most once when label, rotation, fixed, movable, or swappable differs. Technical empty cells and geometry changes are excluded, and an equivalent update remains dirty- and audit-free.

Stage 4C.3 below adds advanced geometry for existing supported Custom elements. Device placement and binding remain outside this scope.

## Stage 4C.3 custom layout advanced geometry

Stage 4C.3 permits span editing only in `custom` layouts and only for teacher desks, doors, windows, walls, aisles, and labels. Student PCs, teacher PCs, printers, network switches, access points, projectors, and explicit empty cells remain strictly 1×1. The canonical validator enforces these persisted invariants without migration or normalization, and geometry mutations never silently convert a named or Grid Classic layout to Custom.

Every element uses a top-left anchor: `rowSpan` extends downward and `columnSpan` extends rightward. A multi-cell element remains one `LayoutElement`, and every coordinate in its rectangular footprint is owned by that element. No explicit empty element may exist underneath it. Growing a footprint consumes only explicit 1×1 empties; shrinking creates deterministic explicit empties for every released coordinate. A collision with any other non-empty footprint, an out-of-bounds request, or a generated-ID conflict rejects the entire immutable operation.

Movement applies the same complete-grid reconciliation. The destination coordinate is the new top-left anchor, overlap between the source's old and new footprints is allowed, destination-only coordinates must be explicit empties, and source-only coordinates become explicit empties. Multi-cell elements never swap; the existing 1×1 student-PC swap is unchanged. The editor resolves drops against logical grid coordinates rather than only rendered empty elements, so an anchor inside the current source footprint remains reachable during a self-overlapping move.

The canvas renders each `LayoutElement` once across its declared CSS-grid row and column spans. Selection and drag hitboxes cover the full footprint. Rotation remains an inner visual transform only and never changes the anchor, span, occupancy, or Device reference. The inspector keeps property and geometry draft actions separate and derives span maxima from the selected anchor and current grid bounds. Permission checks remain enforced in both controls and mutation handlers.

Geometry Apply participates in the existing local draft, dirty fingerprint, Save, Cancel, navigation protection, recovery, and atomic persistence lifecycle. A successful Save still emits exactly one `layout.save` audit and now appends `geometryChanged`, counting each existing non-empty ID once when either span differs. Position remains `repositioned`; technical empty reconciliation does not inflate added or removed counts. Exact geometry reversion is persistence- and audit-free even when technical empty IDs were regenerated.

This stage does not change the schema or external storage version, Device records, Device references, Asset binding, or laboratory Device counts. Device placement, detachment, replacement, transfer, lifecycle states, and unplaced-device workflows remain deferred to Stage 4D.
