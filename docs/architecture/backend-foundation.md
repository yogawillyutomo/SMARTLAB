...
Authentication uses Laravel Sanctum with stateful session-cookie middleware for the first-party SPA and bearer tokens for mobile or API token clients. The implemented SPA entry points are `POST /api/v1/auth/login` and `POST /api/v1/auth/logout`; browser clients first obtain `/sanctum/csrf-cookie`, and successful login rotates the Laravel session identifier. Application identity models use ULIDs, including `User`; Sanctum's `personal_access_tokens.tokenable_id` is an ULID morph key so token authentication remains compatible with that identity type.
...
`packages/contracts/openapi.yaml` is the source of truth for implemented HTTP contracts. It documents only the currently implemented health, SPA session authentication, current-user context, and Laboratory endpoints; it intentionally does not speculate about future resource APIs.
