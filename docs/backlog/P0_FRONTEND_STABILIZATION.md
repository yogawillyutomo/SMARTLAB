...
| AV-01 Unified availability | canonical + Reservation | Laboratory status + active ScheduleOccurrence + schedule coverage + Calendar + submitted/approved Reservation sudah unified. Exception/Priority akan memperluas engine yang sama. | S2.5 + S2.6 delivered; lanjut S2.7 Schedule Exception. |
...
Urutan produk berikutnya tidak lagi mengikuti urutan baseline frontend lama secara literal. Ownership + kontrak S2.1 terkunci, S2.2 backend, S2.3 read-model/frontend cutover, S2.4 Calendar/Closure, S2.5 Unified Availability, dan S2.6 Reservation sudah delivered; pekerjaan berikutnya adalah S2.7 Schedule Exception, bukan solver dan bukan CRUD jadwal browser-local.
