...
| EX-01 Pelaksanaan Lab/laporan terpadu | canonical melalui S3.5; S3.6 in progress di PR #74 | LaboratorySession + ActivityReport + `/sessions` sudah server-authoritative; `/journals` compatibility redirect; S3.5 menambah immutable observations, explicit Incident promotion, dan private report attachments. | S3.6 offline draft sync + operational UAT belum merged; operator browser UAT dan exact merged-head verification tetap menjadi exit gate. |

Urutan produk berikutnya tidak lagi mengikuti urutan baseline frontend lama secara literal. Ownership + kontrak S2.1 terkunci; seluruh S2.2–S2.8 sudah delivered; S3.1–S3.5 sudah merged dan canonical. S3.6 sedang berada di open PR #74 dan belum menjadi authority `main`. Setelah operator UAT, explicit merge decision, dan exact merged-head verification menutup S3.6, fase fungsional berikutnya adalah S4 Assets/Inventory/Loans/Preventive Maintenance. Roadmap current dirangkum di [SMARTLAB Documentation](../README.md).
