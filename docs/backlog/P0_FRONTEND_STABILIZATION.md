...
| AV-01 Unified availability | canonical + Reservation + Exception | Laboratory status + active ScheduleOccurrence + dated Schedule Exception + schedule coverage + Calendar + submitted/approved Reservation sudah unified. Priority Event akan memperluas engine yang sama. | S2.5–S2.7 delivered; lanjut S2.8 Priority Event/Integration UAT. |
| OV-01 Override kegiatan prioritas | sebagian | Fondasi Schedule Exception bertanggal cancel/relocate sudah canonical; Special/Priority Event dan approval policy belum. | S2.7 delivered; Priority Event + policy di S2.8. |
...
Urutan produk berikutnya tidak lagi mengikuti urutan baseline frontend lama secara literal. Ownership + kontrak S2.1 terkunci, S2.2 backend, S2.3 read-model/frontend cutover, S2.4 Calendar/Closure, S2.5 Unified Availability, S2.6 Reservation, dan S2.7 Schedule Exception sudah delivered; pekerjaan berikutnya adalah S2.8 Priority Event + publication impact/reconciliation + integration UAT, bukan solver dan bukan CRUD jadwal browser-local.
