...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (DB::connection()->getDriverName() === 'pgsql') {
            ...
            DB::unprepared(<<<'SQL'
                CREATE OR REPLACE FUNCTION smartlab_validate_work_order_part_usage()
                RETURNS trigger AS $smartlab$
                DECLARE
                    transaction_row inventory_transactions%ROWTYPE;
                    work_order_school char(26);
                BEGIN
                    ...
                    IF NOT FOUND THEN
                        RAISE EXCEPTION 'WorkOrder part usage InventoryTransaction not found';
                    END IF;

                    SELECT school_id INTO work_order_school
                    FROM work_orders
                    WHERE id = NEW.work_order_id;

                    IF work_order_school IS NULL OR work_order_school <> NEW.school_id
                        OR transaction_row.school_id <> NEW.school_id
                        OR transaction_row.inventory_item_id <> NEW.inventory_item_id
                        OR transaction_row.client_mutation_id <> NEW.client_mutation_id
                        OR transaction_row.kind <> 'issue'
                        OR transaction_row.source_type <> 'work_order'
                        OR transaction_row.source_id <> NEW.work_order_id
                        OR transaction_row.quantity <> NEW.quantity
                        OR transaction_row.item_code_snapshot <> NEW.item_code_snapshot
                        OR transaction_row.item_name_snapshot <> NEW.item_name_snapshot
                        OR transaction_row.unit_snapshot <> NEW.unit_snapshot
                        OR transaction_row.actor_user_id_snapshot <> NEW.actor_user_id_snapshot
                        OR transaction_row.actor_membership_id_snapshot <> NEW.actor_membership_id_snapshot
                        OR transaction_row.actor_name_snapshot <> NEW.actor_name_snapshot
                    THEN
                        RAISE EXCEPTION 'WorkOrder part usage must exactly match its canonical InventoryTransaction';
                    END IF;
            ...
            DB::statement('CREATE TRIGGER work_order_part_usages_validate_insert BEFORE INSERT ON work_order_part_usages FOR EACH ROW EXECUTE FUNCTION smartlab_validate_work_order_part_usage()');
        ...
        if (DB::connection()->getDriverName() === 'sqlite') {
            ...
            DB::unprepared("CREATE TRIGGER work_order_part_usages_validate_insert BEFORE INSERT ON work_order_part_usages
                WHEN NOT EXISTS (
                    SELECT 1
                    FROM inventory_transactions t
                    JOIN work_orders w ON w.id = NEW.work_order_id
                    WHERE t.id = NEW.inventory_transaction_id
                      AND w.school_id = NEW.school_id
                      AND t.school_id = NEW.school_id
                      AND t.inventory_item_id = NEW.inventory_item_id
                      AND t.client_mutation_id = NEW.client_mutation_id
                      AND t.kind = 'issue'
                      AND t.source_type = 'work_order'
                      AND t.source_id = NEW.work_order_id
                      AND t.quantity = NEW.quantity
                      AND t.item_code_snapshot = NEW.item_code_snapshot
                      AND t.item_name_snapshot = NEW.item_name_snapshot
                      AND t.unit_snapshot = NEW.unit_snapshot
                      AND t.actor_user_id_snapshot = NEW.actor_user_id_snapshot
                      AND t.actor_membership_id_snapshot = NEW.actor_membership_id_snapshot
                      AND t.actor_name_snapshot = NEW.actor_name_snapshot
                )
                BEGIN SELECT RAISE(ABORT, 'WorkOrder part usage must exactly match its canonical InventoryTransaction'); END");
    ...
    public function down(): void
    {
        ...
        if (DB::connection()->getDriverName() === 'pgsql') {
            ...
            DB::statement('DROP FUNCTION IF EXISTS smartlab_validate_work_order_part_usage()');
        }
    }
};
