...
class UpdateLessonPeriodSetRequest extends FormRequest
{
    use RejectsUnknownFields;

    private const FIELDS = ['name', 'status'];
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            if (array_intersect(array_keys($this->all()), self::FIELDS) === []) {
                $validator->errors()->add('request', 'At least one Lesson Period Set field is required.');
            }
        });
    }
}
