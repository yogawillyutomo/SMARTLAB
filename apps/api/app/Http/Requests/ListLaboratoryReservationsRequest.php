...
class ListLaboratoryReservationsRequest extends FormRequest
{
    use RejectsUnknownFields;

    private const FIELDS = ['from', 'to', 'laboratoryId', 'status', 'scope', 'page', 'perPage'];
    ...
    public function rules(): array
    {
        return [
            ...
            'status' => ['sometimes', Rule::in(['submitted', 'approved', 'rejected', 'cancelled'])],
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            if ($from && $to && $from->diffInDays($to) > 365) {
                $validator->errors()->add('to', 'Reservation range may not exceed 366 calendar days.');
            }
        });
    }
}
