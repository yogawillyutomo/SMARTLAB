          ...
          {notifOpen && (
            <div className="absolute right-0 top-12 z-50 w-80 rounded-xl border border-base-700 bg-base-800 shadow-elevated animate-slide-up">
              <div className="flex items-center justify-between border-b border-base-700 px-4 py-3">
                ...
                <Link to="/notifications" onClick={() => setNotifOpen(false)} className="text-xs text-accent-blue hover:underline">
                  Lihat semua
                </Link>
              </div>
              <div className="max-h-80 overflow-y-auto">
                {db.notifications.slice(0, 6).map((n) => (
                  <Link
                    ...
                    className={cn('flex gap-3 border-b border-base-700/40 px-4 py-3 transition-colors hover:bg-base-700/40', !n.read && 'bg-accent-blue/5')}
                  >
          ...
          {profileOpen && (
            <div className="absolute right-0 top-12 z-50 w-64 rounded-xl border border-base-700 bg-base-800 shadow-elevated animate-slide-up">
              <div className="border-b border-base-700 px-4 py-3">
                ...
                <p className="mt-1 inline-flex rounded-md bg-accent-blue/15 px-2 py-0.5 text-[10px] font-medium text-accent-blue">{user?.role}</p>
              </div>
              <div className="p-2">
                  ...
                  {ROLES.map((r) => (
                    <button
                      ...
                      className={cn(
                        ...
                        user?.role === r ? 'text-accent-blue' : 'text-ink-secondary'
                      )}
                    >
                      {r}
                      {user?.role === r && <span className="h-1.5 w-1.5 rounded-full bg-accent-blue" />}
                    </button>
                  ))}
                </div>
              </div>
