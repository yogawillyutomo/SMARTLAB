        ...
        <Card>
          <CardContent className="space-y-1 p-2">
            {TABS.map((t) => {
              const Icon = t.icon;
              return (
                <button key={t.key} onClick={() => setTab(t.key)} className={cn('flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors', tab === t.key ? 'bg-accent-blue/15 text-accent-blue' : 'text-ink-secondary hover:bg-base-700/60 hover:text-ink-primary')}>
              ...
              <CardContent className="space-y-6">
                ...
                <div>
                  <label className="flex items-center justify-between rounded-lg border border-base-700/60 bg-base-800/40 px-3 py-3">
                    ...
                    <input type="checkbox" checked={compactTable} onChange={(e) => setCompactTable(e.target.checked)} className="h-5 w-5 rounded border-base-600 text-accent-blue" />
                  </label>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-2">
                {['Incident Baru', 'Work Order', 'Maintenance Overdue', 'Stok Rendah', 'Booking Baru', 'Jurnal', 'Peminjaman Terlambat', 'PC Offline'].map((n) => (
                  <label key={n} className="flex items-center justify-between rounded-lg border border-base-700/60 bg-base-800/40 px-3 py-2.5">
                    ...
                    <input type="checkbox" defaultChecked className="rounded border-base-600 text-accent-blue" />
                  </label>
                ))}
              </CardContent>
            </Card>
