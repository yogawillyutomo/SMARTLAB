    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-2 sm:grid-cols-3 lg:grid-cols-5">
        {REPORT_CATS.map((cat) => {
          const Icon = cat.icon;
          return (
            <button key={cat.key} onClick={() => setActive(cat.key)} className={`flex items-center gap-2 rounded-xl border p-3 text-left text-sm transition-all ${active === cat.key ? 'border-accent-primary bg-accent-primary/10 text-accent-primary' : 'border-base-700 text-ink-secondary hover:border-base-600'}`}>
