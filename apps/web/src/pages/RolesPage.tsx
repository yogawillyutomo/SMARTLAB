                  ...
                  {PERMISSION_ACTIONS.map((act) => {
                    ...
                    return (
                      <td key={act} className="px-3 py-2 text-center">
                        <button disabled={!canEdit} onClick={() => toggle(activeRole, mod, act)} className={cn('inline-flex h-6 w-6 items-center justify-center rounded-md border transition-colors disabled:cursor-not-allowed disabled:opacity-50', allowed ? 'border-emerald-500 bg-emerald-500/15 text-emerald-400' : 'border-base-600 text-transparent hover:border-base-500')}>
