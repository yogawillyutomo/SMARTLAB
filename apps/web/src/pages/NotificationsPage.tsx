      ...
      <Card>
        <CardContent className="flex flex-wrap items-end gap-3">
          ...
          <label className="flex items-center gap-2 text-sm text-ink-secondary pb-2.5">
            <input type="checkbox" checked={showUnread} onChange={(e) => setShowUnread(e.target.checked)} className="rounded border-base-600 text-accent-blue" />
            Hanya belum dibaca
          </label>
                ...
                <div key={n.id} className={cn('flex items-start gap-3 p-4 transition-colors hover:bg-base-700/20', !n.read && 'bg-accent-blue/5')}>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {!n.read && <span className="h-2 w-2 rounded-full bg-accent-blue" />}
                    </div>
