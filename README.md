...
- S4.4 implementation candidate in PR #81: exact-Asset Loan/LoanItem custody, ETag lifecycle actions, double-checkout exclusion, condition evidence, OpenAPI 0.27, and server-authoritative `/loans`; it becomes merged authority only after explicit merge and exact merged-head verification;
- Dashboard metrics for laboratories, devices, and incidents.

### Transitional browser-local domains

Pelaksanaan Lab is now server-authoritative through S3.6, including explicit issue evidence, Incident linkage, private report attachments, and controlled offline ActivityReport draft working copies that preserve server/version authority. Remaining transitional work includes telemetry monitoring, work orders, maintenance, notifications, reports, tenant settings, audit-log query UI, and several cross-domain summaries. Inventory/stock is merged and canonical through S4.3. On PR #81, `/loans` is also cut over to exact-Asset server custody and may not fall back to browser-local authority; merged `main` does not gain that S4.4 authority until PR #81 is explicitly merged and the exact merged head is verified. After S4.4 merge verification, the next functional slice is S4.5 Preventive Maintenance. Operator/browser offline UX scenarios remain a production-rollout UAT requirement rather than a reason to weaken server authority.
