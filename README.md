...
- canonical S4.6 reconciliation on merged `main@3757e986`: read-only Asset operational-state projection with provenance, active-custody lifecycle/unlink reconciliation, versioned in-progress Maintenance checklist progress, real PostgreSQL contention tests, S4 source-of-truth closure scans, documentation-link CI, and recorded browser UAT evidence; PR #83 merged with exact merged-head API/web CI green;
- Dashboard metrics for laboratories, devices, and incidents.

### Transitional browser-local domains

Pelaksanaan Lab is server-authoritative through S3.6, and all four S4 routes (`/assets`, `/stock`, `/loans`, `/maintenance`) plus S4.6 reconciliation evidence are canonical on merged `main`. S4 is closed on PR #83 / `3757e986` with browser UAT recorded and exact merged-head API/web CI green. Remaining transitional work includes telemetry monitoring, corrective Work Orders, notifications, reports, tenant settings, audit-log query UI, and several cross-domain summaries. S5 Corrective Work Order authority remains separate; S4 does not create Work Orders or Incidents implicitly.
